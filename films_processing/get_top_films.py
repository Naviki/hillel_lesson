...
from text_proccesing.unique_counter import words_counts
...
def random_rating(rating):
    descriptions = []
    for film in imdb_rank:
        if float(film["rating"]) == rating:
          descriptions.append(film["description"])
    if not descriptions:
        return "Фільму з таким рейтинком немає"
    return random.choice(descriptions)

print(words_counts(random_rating(8.3)))